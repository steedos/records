Meteor.methods
	archive_receive: (object_name,selectedIds,space) ->
		...
		selectedIds.forEach (selectedId)->
			...
			if newSuccessNum
				record = collection.findOne(selectedId,{fields:{fonds_name:1,archival_category_code:1,year:1}})
				sequence = collection.find({year:record?.year,is_received:true},{fields:{_id: 1}}).count()+1
				if record?.fonds_name and record?.archival_category_code and record?.year
					fonds_name_code = Creator.Collections["archive_fonds"].findOne(record.fonds_name,{fields:{code:1}})?.code
					year = record.year
					sequence = (Array(6).join('0') + sequence).slice(-5)
					electronic_record_code = fonds_name_code + "WS" + year + sequence
